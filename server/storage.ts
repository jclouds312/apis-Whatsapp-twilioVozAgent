...
export interface IStorage {
  ...
  getWhatsappMessagesByUser(userId: string, limit?: number): Promise<WhatsappMessage[]>;
  getWhatsappMessagesByPhone(userId: string, phoneNumber: string, limit?: number): Promise<WhatsappMessage[]>;
  ...
  updateCrmContact(id: string, updates: Partial<CrmContact>): Promise<CrmContact | undefined>;
...
export class Storage implements IStorage {
  ...
  async getServiceByName(userId: string, name: string): Promise<Service | undefined> {
    return db.query.services.findFirst({
      where: and(eq(services.userId, userId), eq(services.name, name)),
    });
  }
  ...
  async getApiKeyByService(userId: string, service: string): Promise<ApiKey | undefined> {
  ...
  async getWhatsappMessage(id: string): Promise<WhatsappMessage | undefined> {
    return db.query.whatsappMessages.findFirst({ where: eq(whatsappMessages.id, id) });
  }

  async getWhatsappMessagesByUser(userId: string, limit = 50): Promise<WhatsappMessage[]> {
  ...
  async getWhatsappMessagesByPhone(userId: string, phoneNumber: string, limit = 50): Promise<WhatsappMessage[]> {
    return db.query.whatsappMessages.findMany({
      where: and(eq(whatsappMessages.userId, userId), eq(whatsappMessages.phoneNumber, phoneNumber)),
      limit,
      orderBy: (wm) => wm.createdAt,
    });
  }

  async createWhatsappMessage(insertMsg: InsertWhatsappMessage): Promise<WhatsappMessage> {
    const [msg] = await db.insert(whatsappMessages).values(insertMsg).returning();
    return msg;
  }

  async updateWhatsappMessage(id: string, updates: Partial<WhatsappMessage>): Promise<WhatsappMessage | undefined> {
    const [updated] = await db.update(whatsappMessages).set(updates).where(eq(whatsappMessages.id, id)).returning();
    return updated;
  }

  // ===== TWILIO CALLS =====
  async getTwilioCall(id: string): Promise<TwilioCall | undefined> {
    return db.query.twilioCalls.findFirst({ where: eq(twilioCalls.id, id) });
  }

  async getTwilioCallByCallSid(callSid: string): Promise<TwilioCall | undefined> {
    return db.query.twilioCalls.findFirst({ where: eq(twilioCalls.callSid, callSid) });
  }
  ...
  async updateTwilioCall(id: string, updates: Partial<TwilioCall>): Promise<TwilioCall | undefined> {
    const [updated] = await db.update(twilioCalls).set(updates).where(eq(twilioCalls.id, id)).returning();
    return updated;
  }
  ...
  async getCrmContact(id: string): Promise<CrmContact | undefined> {
    return db.query.crmContacts.findFirst({ where: eq(crmContacts.id, id) });
  }

  async getCrmContactsByUser(userId: string, limit = 100): Promise<CrmContact[]> {
  ...
  async updateCrmContact(id: string, updates: Partial<CrmContact>): Promise<CrmContact | undefined> {
    const [updated] = await db.update(crmContacts).set(updates).where(eq(crmContacts.id, id)).returning();
    return updated;
  }

  // ===== SYSTEM LOGS =====
  async getSystemLogs(userId: string | null, limit = 100): Promise<SystemLog[]> {
    if (userId) {
  ...
  async getExposedApi(id: string): Promise<ExposedApi | undefined> {
    return db.query.exposedApis.findFirst({ where: eq(exposedApis.id, id) });
  }
  ...
  async getExposedApiByEndpoint(endpoint: string): Promise<ExposedApi | undefined> {
    return db.query.exposedApis.findFirst({ where: eq(exposedApis.endpoint, endpoint) });
  }
  ...
  async updateExposedApi(id: string, updates: Partial<ExposedApi>): Promise<ExposedApi | undefined> {
    const [updated] = await db.update(exposedApis).set(updates).where(eq(exposedApis.id, id)).returning();
    return updated;
  }
