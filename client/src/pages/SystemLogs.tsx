          ...
          <div className="rounded-md border border-border/50 overflow-hidden">
            <table className="w-full text-sm text-left">
              ...
              <tbody className="divide-y divide-border/50">
                {isLoading ? (
                  <tr><td colSpan={5} className="px-4 py-8 text-center"><Loader2 className="h-4 w-4 animate-spin inline" /></td></tr>
                ) : logs.slice(-50).map((log: any) => (
                  <tr key={log.id} className="hover:bg-muted/30 transition-colors group">
                    ...
                    <td className="px-4 py-3">
                      <Badge variant="outline" className={`
                        capitalize border-0 
                        ${log.status === 'error' ? 'bg-rose-500/10 text-rose-500' : 
                      ...
                      `}>
                        ...
                        {log.status === 'info' && <Info className="h-3 w-3 mr-1" />}
                        {log.status}
                      </Badge>
                    </td>
                    <td className="px-4 py-3 font-medium capitalize">{log.service || log.eventType}</td>
